...
@section('title', __('roles.edit_user_roles'))
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">{{ __('roles.edit_user_roles') }}</h1>
            <p class="text-gray-600 mt-1">{{ __('roles.manage_roles_for_user', ['user' => $user->name]) }}</p>
        </div>
        <div class="flex space-x-3 {{ app()->getLocale() === 'ar' ? 'space-x-reverse' : '' }}">
            <a href="{{ route('modules.roles.users.index') }}" class="btn-secondary">
    ...
    <!-- User Information -->
    <x-card title="{{ __('roles.user_information') }}">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400">{{ __('roles.user_id') }}: {{ $user->id }}</p>
                </div>
            </div>
            
            <div class="space-y-2">
                <div class="flex justify-between">
                    <span class="text-sm text-gray-500">{{ __('roles.account_status') }}:</span>
                    <span class="text-sm font-medium {{ $user->email_verified_at ? 'text-green-600' : 'text-red-600' }}">
                        {{ $user->email_verified_at ? __('roles.verified') : __('roles.unverified') }}
                    </span>
                </div>
                <div class="flex justify-between">
                    <span class="text-sm text-gray-500">{{ __('roles.joined_date') }}:</span>
                    <span class="text-sm text-gray-900">{{ $user->created_at ? $user->created_at->format('M j, Y') : 'N/A' }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-sm text-gray-500">{{ __('roles.current_roles') }}:</span>
    ...
    <!-- Current Roles -->
    <x-card title="{{ __('roles.current_roles') }}">
        ...
        @else
            <div class="text-center py-8">
                ...
                <h3 class="text-lg font-medium text-gray-900 mb-2">{{ __('roles.no_roles_assigned') }}</h3>
                <p class="text-gray-600">{{ __('roles.user_has_no_roles') }}</p>
            </div>
        @endif
    </x-card>

    <!-- Role Assignment Form -->
    <x-card title="{{ __('roles.assign_roles') }}">
        <form action="{{ route('modules.roles.users.update', $user) }}" method="POST">
            @csrf
            @method('PUT')
            
            <div class="space-y-4">
                <p class="text-gray-600">{{ __('roles.select_roles_to_assign') }}</p>
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="flex items-center space-x-1">
                                        ...
                                        @if($role->inherit_permissions)
                                            <svg class="w-3 h-3 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" title="{{ __('roles.inherits_permissions') }}">
    ...
    @if($user->roles->count() > 0)
        <x-card title="{{ __('roles.effective_permissions') }}">
            <div class="space-y-4">
                <p class="text-gray-600">{{ __('roles.permissions_from_assigned_roles') }}</p>
                ...
                @if(count($uniquePermissions) > 0)
                    ...
                    <div class="text-sm text-gray-500 mt-4">
                        {{ __('roles.total_unique_permissions') }}: {{ count($uniquePermissions) }}
                    </div>
                @else
